...
#     model.eval()
#     tokenizer = model.config._name_or_path  # not used, but keeps structure clear
...
#         for seq in pred_ids:
#             text = args.tokenizer.decode(seq, skip_special_tokens=True)
#             generated_sql.append(text)

#     # ----------------------------
#     # 3. Save SQL queries + compute records
#     # ----------------------------
...
#     # syntax error rate = % of outputs whose SQL failed execution
#     error_count = sum([1 for e in error_msgs if e != ""])
#     error_rate = error_count / len(error_msgs)
...
# def test_inference(args, model, test_loader, model_sql_path, model_record_path):
#     """
#     Identical to eval_epoch but without loss calculation and without ground truth.
#     Saves SQL predictions + records.
#     """
...
#     for encoder_input, encoder_mask, initial_dec_input in tqdm(test_loader):
...
#     # save SQL + compute + save records
#     save_queries_and_records(all_sql, model_sql_path, model_record_path)

#     print(f"[Test inference done] Saved SQL queries to {model_sql_path}")
#     print(f"[Test inference done] Saved record outputs to {model_record_path}")
...
def eval_epoch(args, model, dev_loader, gt_sql_path, model_sql_path,
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader):
        ...
        for seq in pred_ids:
            seq = seq.tolist()
            text = args.tokenizer.decode(seq, skip_special_tokens=True)
            print("DEBUG PRED:", text)     # <---- add this
            generated_sql.append(text)
    ...
    error_rate = sum([1 for e in error_msgs if e != ""]) / len(error_msgs)
    avg_loss = total_loss / total_tokens
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    for encoder_input, encoder_mask, bos in tqdm(test_loader):
    ...
    save_queries_and_records(generated, model_sql_path, model_record_path)
    print(f"[Test inference done] Saved SQL queries to {model_sql_path}")
    print(f"[Test inference done] Saved record outputs to {model_record_path}")
