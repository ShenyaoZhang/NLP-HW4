...
def create_augmented_dataloader(args, dataset):
    ...
    # 2. Sample 5000 examples to transform
    sampled = original_train.shuffle(seed=42).select(range(5000))

    # 3. Apply custom transformation
    transformed = sampled.map(
        custom_transform,
        load_from_cache_file=False
    )

    # 4. Combine: original + transformed
    augmented = datasets.concatenate_datasets([original_train, transformed])

    # 5. Tokenize
    augmented_tokenized = augmented.map(
        tokenize_function,
        batched=True,
        load_from_cache_file=False
    )
